...
(async()=>{
 ...
 for(const file of ['manifest.json','kakashi.webp','naruto.webp','sasuke.webp']){
  const expected=hash(fs.readFileSync(`public/art-platform/${file}`));const response=await p.request.get(new URL(`art-platform/${file}`,url).href);
  assert.equal(response.status(),200);const actual=hash(await response.body());assert.equal(actual,expected);assets[file]=actual;
 }
