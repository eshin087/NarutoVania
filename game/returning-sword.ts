...
export class ReturningSword {
  ...
  step(dt:number,hand:{x:number;y:number}){
    ...
    if(this.phase==='outbound'){
      const distance=Math.min(this.remaining,1100*dt/1000);this.x+=this.dx*distance;this.y+=this.dy*distance;this.remaining-=distance;
      ...
      const dx=hand.x-this.x,dy=hand.y-this.y,d=Math.hypot(dx,dy),step=1250*dt/1000;
