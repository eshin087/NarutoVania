...
import audioManifest from '../public/audio-v11/manifest.json';
import '@/app/boss.css';
...
const controls = [
  ...
  ['Signature techniques', 'Q / E', 'RB / RT'], ['Substitution', 'L', 'LT / L2'], ['Ultimate', 'R', 'Right-stick click'], ['Pause / continue panel', 'Esc / Enter', 'Start / A'],
];
...
export default function BossPage() {
  ...
  const openPanel = (next: 'controls' | 'credits' | 'debug') => {if (['playing', 'intro'].includes(state.screen)) {command('pause'); setResumeAfter(true);} setPanel(next);};
  ...
  return <main className="boss-shell">
    ...
    <section ref={frame} className={`br-frame ${expanded ? 'br-fullscreen' : ''}`} aria-label="Naruto Land of Waves story boss rush">
      ...
      {state.screen === 'intro' && <div className="br-cinema-ui"><div className="br-letterbox br-letterbox-top"/><div className="br-letterbox br-letterbox-bottom"/><span className="br-cinema-chapter">LAND OF WAVES · CHAPTER 01</span><div className="br-cinema-actions">{state.panelWaiting&&<Button className="br-primary" disabled={!state.canAdvance} onClick={()=>command('advance')}>CONTINUE <kbd>{controller?'A':'ENTER'}</kbd><ArrowRight/></Button>}<Button variant="ghost" onClick={()=>command('skip')}>Skip scene</Button></div></div>}
