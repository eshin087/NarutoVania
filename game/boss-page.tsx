...
import {ArrowRight, Volume2, VolumeX, Maximize, Pause, X, Settings2, Gamepad2, Keyboard, Eye, Waves, Dog, Zap, Users, Wind, Flame, Shield, Swords, Disc, Sparkles, RotateCcw} from 'lucide-react';
import {bossBridge as bridge, type Command, type Settings, type Snapshot} from './boss-bridge';
import {CHARACTER, PHASES, PHASE_IDS, STORY_SOURCES, kit, type Ability} from './chapter';
import audioManifest from '../public/audio/manifest.json';
import '@/app/boss.css';

const controls = [
  ['Move / crouch', 'A D / S or arrows', 'Left stick / D-pad'], ['Jump', 'Space', 'A / Cross'],
  ['Melee / charged heavy', 'J / down + hold J', 'X / Square'], ['Ranged tool', 'K', 'Y / Triangle'],
...
function AbilityIcon({name}: {name: Ability['icon']}) {
  const Icon = {eye: Eye, water: Waves, dog: Dog, bolt: Zap, clone: Users, feint: Wind, rush: Sparkles, fire: Flame, shuriken: Disc, kunai: Swords, shield: Shield}[name];
  ...
  return <main className="boss-shell">
    ...
    <section ref={frame} className={`br-frame ${expanded ? 'br-fullscreen' : ''}`} aria-label="Naruto Land of Waves story boss rush">
      ...
      {state.screen === 'title' && <div className="br-title"><div className="br-title-copy"><span className="br-eyebrow"><i/> A BOND FORGED IN BATTLE</span><h1>NARUTO</h1><h2>LAND OF WAVES</h2><div className="br-title-rule"/><p>One team. Seven trials.<br/>A bridge worth fighting for.</p><Button className="br-primary br-start" onClick={() => command(state.seen.length ? 'continue' : 'start')}>{state.seen.length ? 'CONTINUE STORY' : 'BEGIN STORY'}<ArrowRight/></Button>
        ...
        <div className="br-title-bottom"><span className="br-large-number">01</span><div><span className="br-eyebrow">STORY BOSS RUSH</span><span>Zabuza Momochi <i>×</i> Haku</span></div><span className="br-duration">A 15–20 MINUTE CHAPTER</span></div><span className="br-vertical" aria-hidden="true">大切なものを守る</span></div>}
      {active && <>
        <div className="br-combat-hud"><div className="br-fighter-hud"><Image unoptimized width={56} height={60} className="br-portrait" src={`/art-v2/portraits/${state.character}.png`} alt=""/><div className="br-meters"><div className="br-name"><b>{character.name}</b><span>{Math.ceil(state.health)} <small>/ 100</small></span></div><Bar value={state.health} kind="health" label="Player health"/><Bar value={state.stamina} kind={state.guardBroken ? 'broken' : 'stamina'} label="Player stamina"/><div className="br-small-meters"><span>CHAKRA</span><Bar value={state.chakra} kind="chakra" label="Chakra"/><span>{Math.floor(state.chakra)}</span></div></div></div>
          <div className="br-phase-number"><span>{String(current.number).padStart(2, '0')}</span><small>/ 07</small></div>
          {state.boss && <div className="br-fighter-hud br-boss-meters"><div className="br-meters"><div className="br-name"><b>{state.boss.name}</b><span>{Math.ceil(state.boss.health / state.boss.max * 100)}<small>%</small></span></div><Bar value={state.boss.health / state.boss.max * 100} kind="boss-health" label="Boss health"/><Bar value={state.boss.stamina} kind={state.boss.guardBroken ? 'broken' : 'stamina'} label="Boss stamina"/>{state.boss.phase && <span className="br-boss-phase">{state.boss.phase}</span>}</div><Image unoptimized width={56} height={60} className="br-portrait" src={`/art-v2/portraits/${current.boss}.png`} alt=""/></div>}
        </div>
        ...
        <div className="br-bottom-hud"><div className="br-time">CH. 01 <i/> {time}</div><div className="br-hotbar">
          <div className="br-ability" title="Melee / charged heavy"><kbd>{controller ? 'X' : 'J'}</kbd><Swords/></div><div className="br-ability" title="Ranged tool"><kbd>{controller ? 'Y' : 'K'}</kbd><Disc/></div>
          <div className={`br-ability ${state.stamina < 22 ? 'br-unready' : ''}`} title="Dash / air dash / slide"><kbd>{controller ? 'B' : '⇧'}</kbd><Wind/></div><div className="br-ability" title="Parry / hold block"><kbd>{controller ? 'LB' : 'F'}</kbd><Shield/></div>
          <div className={`br-ability ${state.subCooldown || state.chakra < 25 ? 'br-unready' : ''}`} title="Substitution"><kbd>{controller ? 'LT' : 'L'}</kbd><RotateCcw/>{state.subCooldown > 0 && <strong>{state.subCooldown.toFixed(1)}</strong>}</div>
          <span className="br-hotbar-divider"/>{state.abilities.map((ability, index) => <div className={`br-ability ${!ability.ready ? 'br-unready' : ''} ${index === 2 ? 'br-ultimate' : ''} ${index === 2 && ability.ready ? 'br-charged' : ''}`} title={ability.label} key={ability.label}><kbd>{controller ? ['RB', 'RT', 'R3'][index] : ['Q', 'E', 'R'][index]}</kbd><AbilityIcon name={ability.icon}/>{ability.cooldown > 0 && <strong>{ability.cooldown.toFixed(1)}</strong>}{index === 2 && <div className="br-ult-line"><span style={{width: percent(state.ultimate)}}/></div>}</div>)}
        </div><span className="br-device">{controller ? <Gamepad2/> : <Keyboard/>}{controller ? 'CONTROLLER' : 'KEYBOARD'}</span></div>
      </>}
    ...
    <Dialog open={panel !== null} onOpenChange={open => {if (!open) closePanel();}}><DialogContent portalContainer={expanded ? frame : undefined} className={`br-dialog ${panel === 'credits' ? 'br-credits-dialog' : ''}`}>
      ...
      {panel === 'controls' ? <><div className="br-control-grid"><div className="br-control-heading"><b>ACTION</b><span>KEYBOARD</span><span>CONTROLLER</span></div>{controls.map(([action, keyboard, pad]) => <div key={action}><b>{action}</b><span>{keyboard}</span><span>{pad}</span></div>)}</div>
        <div className="br-combat-tips"><p><b>Deflect or guard.</b> Tap F just before a hit to parry. Hold it to block frontal hits. Red attacks require a dodge. Ground and air dashes cost stamina; a slide passes beneath high projectiles and has no invulnerability.</p><p><b>Commit to your strikes.</b> Tap J to buffer the three-hit string. Down + hold J charges a heavy strike. Defense can cancel recovery. Bosses stagger when their stamina breaks.</p></div>
