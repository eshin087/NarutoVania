...
import {CHARACTER, PHASES, PHASE_IDS, STORY_SOURCES, kit} from './chapter';
...
function AbilityIcon({id}: {id:string}) {return <Image unoptimized width={48} height={48} src={`/art-v3/icons/${id}.webp`} alt=""/>;}
  ...
  return <main className="boss-shell">
    ...
    <section ref={frame} className={`br-frame ${expanded ? 'br-fullscreen' : ''}`} aria-label="Naruto Land of Waves story boss rush">
      ...
      {state.screen === 'title' && <div className="br-title"><div className="br-title-copy"><span className="br-eyebrow"><i/> A BOND FORGED IN BATTLE</span><h1>NARUTO</h1><h2>LAND OF WAVES</h2><div className="br-title-rule"/><p>One team. Seven trials.<br/>A bridge worth fighting for.</p><Button className="br-primary br-start" onClick={() => command(state.seen.length ? 'continue' : 'start')}>{state.seen.length ? 'CONTINUE STORY' : 'BEGIN STORY'}<ArrowRight/></Button>
        ...
        <div className="br-title-bottom"><span className="br-large-number">01</span><div><span className="br-eyebrow">STORY BOSS RUSH</span><span>Zabuza Momochi <i>×</i> Haku</span></div><span className="br-duration">SEVEN SHORT, INTENSE DUELS</span></div><span className="br-vertical" aria-hidden="true">大切なものを守る</span></div>}
      {active && <>
