...
export function preloadV15(scene:Phaser.Scene){scene.load.image('v16-mirror','/art-v16/mirror.webp');scene.load.image('v15-water','/art-v15/water.webp');scene.load.image('v15-reactions','/art-v15/reactions.webp');}
...
export function poseRepair(sprite:Phaser.GameObjects.Sprite|Phaser.GameObjects.Image,frame:number,facing:number){sprite.setTexture('v15-reactions',String(frame)).setOrigin(.5,224/256).setScale(1).setFlipX(facing<0);presentBody(sprite,frame>=4&&frame<8?'sasuke':frame>=8&&frame<12?'naruto':'zabuza');}
