...
export interface PreparedVolley{index:number;at:number;targetX:number;gap:{left:number;right:number};lanes:number[];style:BarrageStyle;leftFirst:boolean;spread:number;seed:number;}
...
export function spiritTarget(v:PreparedVolley,_originX:number,floor:number){return{x:(v.gap.left+v.gap.right)/2,y:floor+125};}
