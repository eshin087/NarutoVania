...
export class CinemaPresentation {
  ...
  constructor(private scene: Phaser.Scene) {
    ...
    this.name = scene.add.text(12, 10, '', {fontFamily: 'Arial', fontSize: '14px', fontStyle: 'bold', color: '#56717b'});
    this.line = scene.add.text(12, 10, '', {fontFamily: 'Arial', fontSize: '23px', color: '#102d38', wordWrap: {width: 390}, lineSpacing: 5});
  ...
  cue(cue: CinemaCue, now: number) {
    ...
    if(cue.manga!==undefined){this.panel.setTexture(`manga-${cue.manga}`).setDisplaySize(960,540).setVisible(true);this.panelMat.setVisible(true);this.panelBorn=now;this.exitAt=0;this.panelUntil=Infinity;this.until=Infinity;}
  ...
  dismissPanel(now: number){this.exitAt=now+180;this.panelUntil=this.exitAt;this.until=this.exitAt;}
  exitFinished(now:number){return this.exitAt>0&&now>=this.exitAt;}
  update(now: number, position: (id: CinemaCue['actor']) => {x: number; y: number} | undefined) {
    ...
    if (panelActive) {const alpha = Math.min(1, (now - this.panelBorn) / 250, (this.panelUntil - now) / 180); this.panel.setAlpha(alpha);this.panel.setScale(960/this.panel.width*(1+Math.min(.012,(now-this.panelBorn)/400000))); this.panelMat.setAlpha(alpha);}
    ...
    const nameHeight=this.name.text?this.name.height+4:0;
    this.name.setVisible(!!this.name.text);this.line.setY(10+nameHeight);
    const width = Math.max(this.name.text?this.name.width:0,...this.line.getWrappedText().map(line=>this.line.context.measureText(line).width))+24, height = this.line.height+nameHeight+20;
