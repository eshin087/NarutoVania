...
describe('signature barrages',()=>{
 for(const id of BARRAGE_IDS)it(`${id} locks aim, warns for 650ms and emits once`,()=>{const t=new BarrageTimeline(id,40,1500),seen:number[]=[];for(let ms=0;ms<6800;ms+=10){const before=new Map([...t.prepared].map(([i,v])=>[i,v.targetX]));seen.push(...t.tick(10,ms%2?100:1400).map(v=>v.index));for(const [i,x]of before)expect(t.prepared.get(i)?.targetX).toBe(x);for(const v of t.warnings)expect(v.at-t.age).toBeLessThanOrEqual(650);}expect(seen).toEqual(BARRAGES[id].times.map((_,i)=>i));expect(t.done).toBe(true);expect(BARRAGES[id].duration-BARRAGES[id].recovery).toBe(800);expect(BARRAGES[id].times[0]).toBeGreaterThanOrEqual(800);});
