...
import {poseHakuV13,dragonFrame,dragonHead,hakuThrowHand} from './art-v13';
...
export type CinemaMotion='rescue-shot'|'dragon-clash'|'counter-wave'|'hunter-throw'|'carry';
...
export class CinematicMotionV13{
 shots:Shot[]=[];clock=0;throwAt:number|null=null;thrown=false;collapseAt:number|null=null;clashAt:number|null=null;clashed=false;
 ...
 start(kind:CinemaMotion,at:number){
  ...
  if(kind==='dragon-clash'&&kakashi&&zabuza){this.clashAt=at;this.clashed=false;for(const actor of [kakashi,zabuza]){actor.animation='cast';actor.animationAt=at;actor.settleAt=at+3000;}
   this.dragons=[1,-1].map(dir=>this.scene.add.image(0,0,'v13-dragon','0').setScale(.5).setFlipX(dir<0).setDepth(5));
  ...
  if(kind==='counter-wave'&&kakashi&&zabuza)this.shot(kakashi.sprite.x+60,this.floor-70,zabuza,-45,-70,'counter');
 ...
 private shot(x:number,y:number,target:Actor,dx:number,dy:number,kind:Shot['kind']){
  const image=kind==='senbon'?this.scene.add.image(x,y,'v8-needle').setDisplaySize(49,7):kind==='counter'?this.scene.add.image(x,y,'v11-wave','0').setOrigin(.8,.7).setDisplaySize(510,204):namedArt(this.scene,'props','shuriken',x,y,32);
 ...
 update(clock:number){
  ...
  this.shots=this.shots.filter(s=>{
   if(s.motion.tick(dt)){
    ...
    if(s.kind==='counter'){s.target.animation='hurt';s.target.animationAt=clock;s.target.settleAt=clock+500;this.scene.tweens.add({targets:s.target.sprite,x:1280,duration:450,ease:'Cubic.easeOut'});this.sounds.softWater();}
   }
   ...
   if(s.kind==='counter')s.image.setFrame(String(Math.min(7,Math.floor(s.motion.age/120)))).setAlpha(s.motion.contact?Math.max(0,1-s.motion.contactAge/480):1);
  ...
  if(this.clashAt!==null){const age=clock-this.clashAt,stage=clashStage(age),frame=dragonFrame(age),center=780;
   if(stage==='formation'||stage==='surge')this.dragons.forEach((dragon,i)=>{const facing=i===0?1:-1,origin=i===0?430:1130,progress=Math.max(0,Math.min(1,(age-700)/700)),headX=origin+(center-origin)*progress,offset=dragonHead(frame,facing);dragon.setFrame(String(frame)).setPosition(headX-offset.x,this.floor-215-offset.y).setAlpha(Math.min(1,age/220));});
