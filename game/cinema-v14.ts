...
export class CinemaPresentation{
 ...
 constructor(private scene:Phaser.Scene){
  this.ink=scene.add.graphics();this.name=scene.add.text(12,10,'',{fontFamily:'Arial',fontSize:'14px',fontStyle:'bold',color:'#56717b'});
  this.line=scene.add.text(12,10,'',{fontFamily:'Arial',fontSize:'22px',color:'#102d38',wordWrap:{width:390},lineSpacing:4});
 ...
 update(now:number,position:(id:CinemaCue['actor'])=>{x:number;y:number}|undefined){
  ...
  const actor=position(this.speaker),camera=this.scene.cameras.main,anchor=new Phaser.Math.Vector2();
  ...
  const x=Phaser.Math.Clamp(anchor.x-width/2,22,1258-width),y=Phaser.Math.Clamp(anchor.y-170*camera.zoom-height,120,570-height);
