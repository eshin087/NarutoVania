...
export const UNIVERSAL: Record<string, AttackDefinition> = {
  ...
  aerial: {id: 'aerial', action: 'aerial', animation: 'aerial', duration: 387, cancelAt: 257, stamina: 7, move: 55,
...
export class Combatant {
  ...
  update(now: number, dt: number): {event: AttackEvent; key: string; charge: number}[] {
    ...
    if (this.isBoss && this.guardBrokenUntil && now >= this.guardBrokenUntil) {this.stamina = 100; this.guardBrokenUntil = 0; this.resolveUntil = now + 1700;}
    if (!this.guard && this.chargeStarted === null && now - this.lastSpend >= COMBAT.regenDelay && now >= this.guardBrokenUntil) {
      this.stamina = Math.min(100, this.stamina + (this.isBoss ? 26 : COMBAT.staminaRegen) * dt / 1000);
    }
  ...
  exhaust(amount: number, now: number) {
    ...
    this.stamina = Math.max(0, this.stamina - amount * (this.isBoss && now < this.resolveUntil ? .35 : 1)); this.lastSpend = now;
  ...
  deflected(amount: number, now: number) {
    ...
    // Guard depletion interrupts a string; individual deflections preserve its next strike.
    this.deflectUntil = now + 160;
  }
  ...
  receive(hit: IncomingHit, now: number): DefenseOutcome {
    ...
    if (!hit.red && frontal && this.guard && now >= this.guardBrokenUntil) {
      if (this.stamina > 0 && now >= this.parryAt && now - this.parryAt < COMBAT.parryWindow) {
        ...
        this.ultimate = Math.min(100, this.ultimate + (this.isBoss?0:16)); this.chakra = Math.min(100, this.chakra + COMBAT.parryChakra);
  ...
  takeDamage(amount: number, now: number, posture = 0, projectile = false) {
    ...
    // Committed boss attacks have armor. A depleted guard interrupts them;
    ...
    else if (!projectile && !committed && now - this.lastStagger >= 1100 && now >= this.guardBrokenUntil && now >= this.resolveUntil) this.stagger(now, posture >= 30 ? 260 : 150);
    return damage;
  }
