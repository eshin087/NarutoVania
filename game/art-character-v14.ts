import {normalizeV11} from './art-v11';
...
export function poseCharacterFrame(sprite:Phaser.GameObjects.Sprite,id:CharacterId,sheet:string,index:number,facing:number,variant?:string){
 if(id==='zabuza'&&sheet==='melee'){const set=Math.floor(index/18)%2,strike=Math.floor(index%18/6),rows=[[0,1,3],[1,3,2]];sprite.setTexture('v8-sword',String(rows[set][strike]*6+index%6)).setFlipX(facing<0);normalizeV11(sprite,id,facing);return true;}
...
export function poseCharacter(sprite:Phaser.GameObjects.Sprite,id:CharacterId,animation:AnimationName,age:number,facing:number,duration?:number,variant?:string){
 ...
 if(animation==='land'&&id!=='zabuza')index=14+Math.min(1,Math.floor(Math.max(0,age)/72.5));
