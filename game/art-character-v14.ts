...
export function poseCharacterFrame(sprite:Phaser.GameObjects.Sprite,id:CharacterId,sheet:string,index:number,facing:number,variant?:string){
 ...
 sprite.setTexture(`v14-${a.key}`,String(index)).setOrigin(facing<0?1-x/w:x/w,y/h).setFlipX(facing<0).setScale(CHARACTER[id].height/a.standingBodyHeight);return true;
}
export function poseCharacter(sprite:Phaser.GameObjects.Sprite,id:CharacterId,animation:AnimationName,age:number,facing:number,duration?:number,variant?:string){
 ...
 if(animation==='idle')index=Math.floor(Math.max(0,age)%1200/200);
 else if(animation==='run')index=6+Math.floor(Math.max(0,age)%(duration||510)/(duration||510)*8);
 else if(animation==='land')index=14+Math.min(1,Math.floor(Math.max(0,age)/72.5));
