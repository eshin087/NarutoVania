...
describe('character scale and hand-bound water launches',()=>{
 ...
 it('retains individual scale and attachment entries for both complete combo sets',()=>{for(const list of Object.values(frames)){expect(list).toHaveLength(36);for(const f of list){expect(f.scale).toBeGreaterThan(0);expect(f.foot).toHaveLength(2);expect(f.head).toHaveLength(2);expect(f.hand).toHaveLength(2);}}});
});
