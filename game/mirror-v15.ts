...
export function fitMirror(scene:Phaser.Scene,image:Phaser.GameObjects.Image,reflection:Phaser.GameObjects.Sprite,kind:Kind){
 ...
 const width=Math.max(base.width,(bounds.width+6)/iw),height=Math.max(base.height,(bounds.height+8)/ih);
 ...
 reflection.x+=innerLeft+iw*width/2-bounds.centerX;
 reflection.y+=insideTop+ih*height-4-bounds.bottom;
}
...
export function mirrorVisual(scene:Phaser.Scene,x:number,y:number,floor:number,kind:Kind,depth=2){
 ...
 poseReflection(reflection,image,kind,false,0,x>830?-1:1);
