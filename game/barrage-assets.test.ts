...
it('places spirit endpoints below the arena so ground routes remain clear',()=>{for(const x of [92,780,1490])for(const origin of [240,1310]){const v=prepareVolley(origin>x?1:0,900,x,70,1520),target=spiritTarget(v,origin,592);expect(target.x<v.gap.left||target.x>v.gap.right).toBe(true);expect(target.y).toBeGreaterThan(622);}});
