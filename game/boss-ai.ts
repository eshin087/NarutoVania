...
function move(id: string, duration: number, stamina: number, events: AttackEvent[], options: Partial<BossMove> = {}): BossMove {
  ...
  const result: BossMove = {id, action: 'boss', animation: 'light1', duration, cancelAt: duration, stamina, events,

    minRange: 0, maxRange: 230, weight: 1, recovery: 750, cooldown: 3500, ...options};

  result.duration = Math.round(duration * .82); result.cancelAt = result.duration;

  result.events = events.map(e => ({...e, at: Math.round(e.at * .82)}));

  result.recovery = Math.max(450, Math.round(result.recovery * .72));

  result.stamina = Math.round(stamina * .8);
...
export class MirrorFormation {
  ...
  transfer(now: number, random: () => number = Math.random) {
    ...
    this.exposedUntil = now + 1600; this.transfers++; return this.mirrors[this.occupied];
