...
describe('returning sword',()=>{
 it('travels fast, waits at turnaround, and catches the moving owner',()=>{const s=new ReturningSword(100,100,1,0,220);s.step(100,{x:100,y:100});expect(s.x).toBe(237.5);s.hit.add('player');s.step(100,{x:100,y:100});expect(s.phase).toBe('turnaround');s.step(119,{x:100,y:100});expect(s.phase).toBe('turnaround');s.step(1,{x:100,y:100});expect(s.hit.size).toBe(0);s.step(100,{x:150,y:100});expect(s.x).toBe(163.75);s.step(100,{x:160,y:105});expect(s.phase).toBe('caught');expect(s.x).toBe(160);expect(s.y).toBe(105);});
 it('deflection disables both remaining passes and returns immediately',()=>{const s=new ReturningSword(400,100,1,0,500);s.parry();expect(s.step(100,{x:100,y:100}).damaging).toBe(false);expect(s.x).toBe(243.75);s.step(200,{x:100,y:100});expect(s.phase).toBe('caught');});
