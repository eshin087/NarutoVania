...
describe('automatic story scenes and isolated scene selection',()=>{
 ...
 it('finishes the snowy tableau automatically only after its full duration',()=>{const d=make();d.startScene('snow');d.update(d.clip!.duration-1);expect(d.state.complete).toBe(false);expect(d.waiting).toBe(false);d.update(1);expect(d.state.complete).toBe(true);});
