...
describe('stable animation and explicit projectile presentation',()=>{
 ...
 it('aims dragon heads outside the guaranteed route rather than erasing them inside it',()=>{for(const x of [92,780,1490])for(const origin of [240,1310]){const v=prepareVolley(0,900,x,70,1520),t=spiritTarget(v,origin,590);expect(t.x<v.gap.left||t.x>v.gap.right).toBe(true);}});
});
