...
Public release `a3d19da` passed fresh browser smoke at `https://narutovania.vercel.app/`: both chapter entries, Chapter2 story/ultimate/controls/controller/modal/focus/resize/debug save isolation, all four Chapter1 fight initializations, no missing assets/errors and no development pilot. Separate check confirmed new controls and exact SHA256 parity of both generated atlases with the validated candidate. Provider success and release evidence are in [the production log](PRODUCTION_LOG.md).
